...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1MR2V,FD', options ) );
	}
	if ( hasOwnProp( options, 'sep' ) ) {
		...
		if (
			...
			return new TypeError( format( '1MRAB,PV', 'sep', opts.sep ) );
		}
	}
	if ( hasOwnProp( options, 'objectMode' ) ) {
		opts.objectMode = options.objectMode;
		if ( !isBoolean( opts.objectMode ) ) {
			return new TypeError( format( '1MR2o,GE', 'objectMode', opts.objectMode ) );
		}
	}
	if ( hasOwnProp( options, 'writableObjectMode' ) ) {
		...
		if ( !isBoolean( opts.writableObjectMode ) ) {
			return new TypeError( format( '1MR2o,GE', 'writableObjectMode', opts.writableObjectMode ) );
		}
	}
	if ( hasOwnProp( options, 'encoding' ) ) {
		opts.encoding = options.encoding;
		if ( !isString( opts.encoding ) ) {
			return new TypeError( format( '1MR2W,Gh', 'encoding', opts.encoding ) );
		}
	}
	if ( hasOwnProp( options, 'allowHalfOpen' ) ) {
		opts.allowHalfOpen = options.allowHalfOpen;
		if ( !isBoolean( opts.allowHalfOpen ) ) {
			return new TypeError( format( '1MR2o,GE', 'allowHalfOpen', opts.allowHalfOpen ) );
		}
	}
	if ( hasOwnProp( options, 'highWaterMark' ) ) {
		opts.highWaterMark = options.highWaterMark;
		if ( !isNonNegative( opts.highWaterMark ) ) {
			return new TypeError( format( '1MR4k,I9', 'highWaterMark', opts.highWaterMark ) );
		}
	}
	return null;
}
